...
def main():
    ...
    output_dir = "/g/data/gi52/jaime/clinvar/run11_ms/five_panlethal_genes"
    ...
    five_genes = ["AURKB", "PLK1", "TDP1", "CDC7", "RPS3"]
    
    # get only these genes
    results = []
    df = df[df["GeneSymbol"].isin(five_genes)].copy()

    # load protein sequences
    protein_seqs = pd.read_csv(protein_seq_csv)

    # write heading to new file
    final_path = os.path.join(output_dir, "five_panlethal_genes_clinvar_csm_scores.csv")
    with open(final_path, "w") as f:
        f.write("Name,GeneSymbol,HGNC_ID,ClinicalSignificance,ProteinChange,esm_score,csm_score\n")

    # process each gene 
    for i, gene in enumerate(five_genes):
        print(f"Processing gene {i+1}/{len(five_genes)}: {gene}", flush=True)
