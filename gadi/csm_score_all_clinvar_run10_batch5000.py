...
def main():
    ...
    run = "run10_ms"
    adapter_path = "/g/data/gi52/jaime/trained/esm2_650M_model/missense/run10/epoch0_batch5000"
    ...
    for i, row in uni_mutations.itertuples():
        ...
        if (i + 1) % 5000 == 0:
            ...
            uni_mutations.to_csv(f"/g/data/gi52/jaime/clinvar/{run}/all_clinvar_esm_scores_intermediate{idx}.csv", index=False)
            torch.cuda.empty_cache()
