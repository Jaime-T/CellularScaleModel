...
"""
Compute initial validation loss on missense mutation dataset
before any training is performed.
...
def main():
    ...
    mut_loss = compute_loss(base_model, DataLoader(mut_valid_data, batch_size=batch_size), device="cuda" if torch.cuda.is_available() else "cpu")  
    wt_loss = compute_loss(base_model, DataLoader(wt_valid_data, batch_size=batch_size), device="cuda" if torch.cuda.is_available() else "cpu")   
    # Print losses
    print(f"Mutant Validation Loss: {mut_loss}")
    print(f"Wildtype Validation Loss: {wt_loss}")
