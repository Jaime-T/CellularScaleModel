...
def train_model(accelerator, tokenizer, model, descr, train_dataset, lora_config, batch_size=6, epochs=3, lr=5e-5):
    ...
    base_dir = f"/g/data/gi52/jaime/trained/esm2_650M_model/{descr}/acc"
    if accelerator.is_main_process:
        os.makedirs(base_dir, exist_ok=True)

    loss_per_epoch = []
    for epoch in range(epochs):
        accelerator.print(f"Starting training for epoch {epoch+1}...")
        model.train()
        total_loss = 0
        ...
        accelerator.print(f"[{descr}] Epoch {epoch+1} training loss: {avg_loss:.4f}")
...
def main():
    ...
    accelerator.print(f"loaded esm2 tokenizer from {model_path}")

    fs_tokenized_df = tokenize_and_mask_seqs(fs_train_df, tokenizer, window_size)
    fs_train_dataset = TorchDataset(fs_tokenized_df)
    accelerator.print(f"Number of fs training samples: {len(fs_train_dataset)}")
    ...
    train_model(accelerator, tokenizer, model, descr, fs_train_dataset, lora_config, batch_size)
